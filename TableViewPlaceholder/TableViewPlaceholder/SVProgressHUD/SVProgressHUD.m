...
+ (void)showWithStatusLight:(NSString*)status{
...
- (void)showProgress:(float)progress status:(NSString*)string{
    ...
    self.style = SVProgressHUDStyleDark;
...
- (void)showImage:(UIImage*)image status:(NSString*)string duration:(NSTimeInterval)duration{
    ...
    self.maskType = SVProgressHUDDefaultMaskType;
    self.style = SVProgressHUDStyleDark;//设置背景颜色样式
                     ...
                     completion:^(BOOL finished){
                         ...
                         if(strongSelf){
                             if(strongSelf.alpha == 0.0f || strongSelf.hudView.alpha == 0.0f){
                                 ...
                                 //DLLog(@"%@", [UIApplication sharedApplication].windows);
                                 //DLLog(@"keyWindow = %@", [UIApplication sharedApplication].keyWindow);
                             }
                         }
                     }];
}
